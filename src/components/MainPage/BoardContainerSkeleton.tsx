			...
			{Array.from({ length: 13 }).map((_, idx) => (
				<div key={idx}>
					...
					<div className={cn("h-[4rem] w-[38.8rem]", "bg-white", "border border-gray-300", "p-[1rem]")}>
						<div className={cn("shimmer h-[1.8rem] w-[15rem] animate-pulse rounded-[0.4rem] bg-gray-200")} />
					</div>
					{/* 게시글 */}
					<div className={cn("h-[15.2rem] w-[38.8rem] bg-gray-100", "border-r border-b border-l border-gray-300")}>
						{Array.from({ length: 4 }).map((_, idx) => (
							...
							>
								<div
									className={cn("shimmer animate-pulse", "h-[1.8rem] w-[36.8rem] rounded-[0.4rem] bg-gray-200")}
								></div>
							</div>
						))}
					</div>
